...
\begin{frame}{Why log--log linearises a power law (\& finite-size effects)}
  ...
  \emph{asymptotic} --- at small $r$ there are corrections.
...
\begin{frame}{Universality across lattices}
  ...
  \TODO{Three lattices, same slope --- the fractal dimension $d_f$ is
        a \emph{universal} quantity. What is non-universal is the
...
\begin{frame}{The running example, revisited}
  ...
  \TODO{Same image as in §2 --- familiar on purpose. The switch we make
        \emph{now} is from a physicist's reading (``looks linear'') to
...
\begin{frame}{What physicists measure vs.\ what they want}
  ...
  \vspace{0.8em}
  \textbf{Working hypothesis.} The population mean admits a $J$-order
  asymptotic expansion
  \[
    \E Y_i \;=\; a_0\, i^{\gamma}\,
       \exp\!\Big(\, \sum_{j=1}^{J} a_j\, i^{-j}
                  \;+\; \phi_J(i)\, i^{-J-1} \,\Big),
    \qquad \phi_J \text{ bounded.}
  \]
  Today: $J = 1$. Identifications for our running example:
  $Y_i \leftrightarrow V(r)$ with $i = r$, $\gamma \leftrightarrow d_f$.

  \TODO{$a_0$ is a lattice-dependent amplitude (the intercept on the
        log--log panel). $a_1, a_2, \ldots$ are finite-size correction
        coefficients --- they encode boundary / discreteness effects.
        $\phi_J$ is the remainder, assumed only \emph{bounded}.
        $J$ controls how many correction terms we keep. $J = 1$ is
        already enough for the running example because the first
        correction $a_1/i$ dominates and the rest is geometric in $i$.
        General $J$ is in the paper~\cite{bigCluster} but does not
        change the structure of the talk.}
\end{frame}
...
\begin{frame}{Bias--variance trade-off}
  ...
  \begin{center}
    \large
    $\Rightarrow$ Choosing \emph{which scales to use} is a
  ...
  \TODO{This is what the rest of the talk is about. The estimator we
        ...
        The whole point of §6 is: \emph{how should $m_0$ scale with
        $m$ and with the compute budget?} That is the practical knob.}
\end{frame}

\begin{frame}{Chalkboard: log--log linearisation}
  Take $\log$ of the $J = 1$ expansion and substitute $i = \rho^k$:
  \[
    \log \E Y_{\rho^k} \;=\;
      \underbrace{\log a_0 \;+\; k\, \gamma \log \rho}_{\text{signal (linear in }k\text{)}}
      \;+\;
      \underbrace{a_1\, \rho^{-k} \;+\; \phi_1(\rho^k)\, \rho^{-2k}}_{\text{finite-size correction (geometric in }k\text{)}}.
  \]

  \vspace{0.4em}
  \begin{itemize}
    \item In the variable $k$ (not $r$!), the leading term is
          \emph{linear} with slope $\gamma \log \rho$.
    \item The correction decays \emph{geometrically} in $k$ (since
          $\rho > 1$): after a few scales it is statistical noise, not
          signal.
    \item $\Rightarrow$ OLS slope of $\log \overline Y_{\rho^k}$ on $k$
          is a natural estimator of $\gamma \log \rho$, hence of
          $\gamma$.
  \end{itemize}

  \TODO{Board steps:
        (1) Start from $\E Y_i = a_0\, i^{\gamma}\, \exp(a_1/i + \phi_1(i)/i^2)$.
        (2) Take log: $\log \E Y_i = \log a_0 + \gamma \log i + a_1/i + \phi_1(i)/i^2$.
        ...
        \emph{exponentially fast} in $k$ --- the whole reason for
