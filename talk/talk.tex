...
\begin{frame}{The three-part split}
  ...
  $x_k = \overline Y_{\rho^k}/\E Y_{\rho^k} - 1$. On the event
  $\{|x_k| \le 1/2\}$,
  $\log(1 + x_k) = x_k - \tfrac12 x_k^2 + O(x_k^3).$
  ...
  $\{x_k\}$; $Q$ is the OLS slope of $\{-\tfrac12 x_k^2 + \cdots\}$.
  ...
  \TODO{The whole §5 proof is one sentence: control $R$, $S$, $Q$
        ...
        $\{|x_k| \le 1/2\}$; the bad event has exponentially small
        probability.}
\end{frame}
...
\begin{frame}{Linear regression as a linear functional}
  ...
  \vspace{0.4em}
  Two weight identities (direct computation from the closed form):
  \begin{itemize}
    \item $\displaystyle \sum_{k = m_0 + 1}^{m_0 + m} w_{k,m}
              \;=\; 0$
          \hfill (kills any constant offset, e.g.\ $\log a_0$)
    \item $\displaystyle \sum_{k = m_0 + 1}^{m_0 + m} k\, w_{k,m}
              \;=\; 1$
          \hfill (picks out the slope coefficient)
  \end{itemize}

  \vspace{0.4em}
  Useful consequence (variance scaling):
  \[
